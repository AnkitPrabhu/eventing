...
.controller('ViewCtrl', ['$q', '$scope', '$rootScope', '$state', '$uibModal', '$timeout', '$location','ViewService', 'serverNodes', 'isEventingRunning',
                         function($q, $scope, $rootScope, $state, $uibModal, $timeout, $location, ViewService, serverNodes, isEventingRunning) {
                         var self = this;
                         self.errorState = !ViewService.status.isErrorCodesLoaded();
                         self.errorCode = 200;
                         self.showErrorAlert = self.showSuccessAlert = false;
                         self.isEventingRunning = isEventingRunning;
                         self.appList = ViewService.local.getAllApps();
                         self.disableEditButton = false;
                         $rootScope.$broadcast('isEventingRunning', self.isEventingRunning);
                         self.isAppListEmpty = function() {
                            return Object.keys(self.appList).length === 0;
                         ...
                         .then(function(response) {
                                //Check if its in some index files
                               return ViewService.tempStore.deleteApp(appName);
                               })
                         .then(function(response) {
                               ...
                               //Check if its using some index files
                               return ViewService.primaryStore.deleteApp(appName);
                               })
                         .then(function(response) {
                               // Delete the local copy of the app in the browser
                               ViewService.local.deleteApp(appName);
                               showSuccessAlert(`${appName} deleted successfully!`);
                               })
                         .catch(function(errResponse) {
                                console.error(errResponse);
...
.controller('ViewHeaderCtrl', ['$q', '$scope', '$uibModal', '$state', 'mnPoolDefault', 'ViewService',
                           function($q, $scope, $uibModal, $state, mnPoolDefault, ViewService) {
                           var self = this;
                           self.isEventingRunning = true;
                           // Subscribe to channel 'isEventingRunning' from SummaryCtrl
                           $scope.$on('isEventingRunning', function(event, args) {
...
.controller('ViewHandlerCtrl', ['$q', '$uibModal', '$timeout', '$state', '$scope', '$stateParams', 'ViewService',
                            function($q, $uibModal, $timeout, $state, $scope, $stateParams, ViewService) {
                            var self = this,
                            isDebugOn = false,
                            debugScope = $scope.$new(true),
                            ...
                            self.debugApp = function() {
                            ViewService.primaryStore.getDeployedApps()
                            .then(function(response) {
                                  if (!(app.appname in response.data)) {
                                  showErrorAlert(`Function ${app.appname} may be undergoing bootstrap. Please try later.`);
                                  return;
                                  ...
                                  // Starts the debugger agent.
                                  ViewService.debug.start(app.appname)
                                  .then(function(response) {
                                        var responseCode = ViewService.status.getResponseCode(response);
                                        if (responseCode) {
                                        var errMsg = ViewService.status.getErrorMsg(responseCode, response.data);
                                        return $q.reject(errMsg);
                                        }
                                        ...
                                        function getDebugUrl() {
                                        console.log('Fetching debug url for ' + app.appname);
                                        ViewService.debug.getUrl(app.appname)
                                        .then(function(response) {
                                              var responseCode = ViewService.status.getResponseCode(response);
                                              if (responseCode) {
                                              var errMsg = ViewService.status.getErrorMsg(responseCode, response.data);
                                  ...
                                  function stopDebugger() {
                                  return ViewService.debug.stop(app.appname)
                                  .then(function(response) {
                                        var responseCode = ViewService.status.getResponseCode(response);
                                        if (responseCode) {
                                        var errMsg = ViewService.status.getErrorMsg(responseCode, response.data);
...
.factory('ViewService', ['$q', '$http', '$state', 'mnPoolDefault',
                         ...
                         .then(function getAppTempStore(response) {
                               ...
                               errHandler.tryResolve(function() {
                                                     return $http.get('/_p/event/getTempView/');
                                                     }, getViewTempStore, errMsg, responseCode);
                               return $q.reject(errMsg);
                               }
                         ...
                         deleteApp: function(appName) {
                         return $http.get('/_p/event/deleteViewLibrary/?name=' + appName);
                         },
...
.config(['$stateProvider', '$urlRouterProvider', 'mnPluggableUiRegistryProvider',
         ...
         .state('app.admin.eventing.views', {
                url: '/Views',
                templateUrl: '../_p/ui/event/ui-current/fragments/views.html',
                controller: 'ViewCtrl',
                controllerAs: 'viewCtrl',
                resolve: {
                loadApps: ['ViewService',
                           function(ViewService) {
                           return ViewService.local.loadApps();
                           }
                           ],
                serverNodes: ['ApplicationService',
                              function(ApplicationService) {
                ...
                isEventingRunning: ['ViewService',
                                    function(ViewService) {
                                    return ViewService.server.isEventingRunning();
                                    }
                                    ]
                }
                })
         ...
         .state('app.admin.eventing.handler', {
                ...
                loadApps: ['ApplicationService',
                           function(ApplicationService) {
                           console.log(ApplicationService.local.loadApps());
                           return ApplicationService.local.loadApps();
                           }
                           ]
                },
         ...
         .state('app.admin.eventing.Viewhandler', {
                url: '/handler/:appName',
                templateUrl: '../_p/ui/event/ui-current/fragments/map-handler.html',
                resolve: {
                loadApps: ['ViewService',
                           function(ViewService) {
                                console.log(ViewService.local.loadApps());
                                return ViewService.local.loadApps();
                           }
                           ]
                },
                data: {
                child: 'app.admin.eventing.views'
                }
                })
